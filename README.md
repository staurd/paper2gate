...
Each run writes `run_summary.json` with stage status. `passed`, `failed`, `skipped`, `unverified`, and `unavailable` are kept distinct. Vivado is optional for generation and simulation, but resource results are unavailable without it.
